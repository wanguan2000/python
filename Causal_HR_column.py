...
class RNA_groupsurvival:
    ...
    def sksurv_compare1(self, mydata):
        mydata3 = mydata[['death_observed', 'survivalMonth', 'myfpkm_level']]

        survival_label = 1
        group_indicator = mydata3[['myfpkm_level']]
        mycount = (mydata3.myfpkm_level.value_counts().to_dict())

        # mydata2.to_csv('1.xls',index=False,sep='\t')

        data_x, data_y = get_x_y(mydata3, ['death_observed', 'survivalMonth'], pos_label=survival_label,
                                 survival=True)
        try:
            chisq, pvalue, stats, covar = compare_survival(data_y, group_indicator, return_stats=True)
            stats2 = stats.iloc[[0, -1]]
            ...
            Eb = stats2.iloc[0, 2]
            # HR = (Oa / Ea) / (Ob / Eb)
            HR = (Oa * Eb) / (Ea * Ob + 0.00001)

        except:
            pvalue = 1
            stats = pd.DataFrame({"L0": [1, 1, ], "L1": [1, 1, ]}, )
            HR = 1

        return HR, pvalue, stats, mycount[0], mycount[1]
    ...
    def distrution_pipeline(self, gene, gene_FPKM, mydata_factor):
        ...
        ttest = scipy.stats.ttest_ind(positive_group.fpkm, negative_group.fpkm, equal_var=False)

        ks_2samp_pvalue = 1
        try:
           ks_2samp = scipy.stats.ks_2samp(positive_group.fpkm, negative_group.fpkm)
           ks_2samp_pvalue = ks_2samp.pvalue
        except:
            ks_2samp_pvalue = 1

        negative_numpy = negative_group.fpkm.to_numpy()
        ...
        positive_numpy = positive_group.fpkm.to_numpy()
        ...
        stat_df = collections.OrderedDict(
            {'gene': [gene], 'pearson': [pearson], 'spearman': [spearman], 'allq10': [allq10], 'allq25': [allq25],
             ...
             'mylevellow_std': [np.std(negative_group.fpkm.to_numpy())],
             'mylevellow_q25': [np.quantile(negative_group.fpkm.to_numpy(), 0.25)],
             'mylevellow_median': [np.median(negative_group.fpkm.to_numpy())],
             'mylevellow_q75': [np.quantile(negative_group.fpkm.to_numpy(), 0.75)],
             'mylevellow_q95': [np.quantile(negative_group.fpkm.to_numpy(), 0.95)],
             'mylevelhigh_std': [np.std(positive_group.fpkm.to_numpy())],
             'mylevelhigh_q25': [np.quantile(positive_group.fpkm.to_numpy(), 0.25)],
             'mylevelhigh_median': [np.median(positive_group.fpkm.to_numpy())],
             'mylevelhigh_q75': [np.quantile(positive_group.fpkm.to_numpy(), 0.75)],
             'mylevelhigh_q95': [np.quantile(positive_group.fpkm.to_numpy(), 0.95)],
