...
def myrwaddR(a, b):
    ...
    waddR_pd = pd.DataFrame(np.array([np.array(waddR_re)]), columns=[
                            'pval', 'dwass2', 'perc_loc', 'perc_size', 'perc_shape'])
    return waddR_pd
...
def logrank_test(d_myfactor, d_survivalMonth, d_death_observed, d_mylength):
    ...
    chisq = (d2i_deathtotal - T2i_sum) * \
        (d2i_deathtotal - T2i_sum) / (V2i_sum + 0.00001)
...
class RNA_groupsurvival:
    def __init__(self, mydata_survival, nstep=1000, bw_adjust=0.5, eps_score=10, my_min_samples=7, pvaluecut=0.2, HRcut=0.7, cut_pos_1=7, cut_pos_0=7, cut_neg_1=7, cut_neg_0=7):
        self.mydata_survival = mydata_survival
        self.nstep = nstep
    ...
    def get_peakdiff(self, negative_density, positive_density, negative_numpy, x_lab):
        ...
        diff_negative_peaks, diff_negative_peakheights = find_peaks(
            diff_negative_positive, height=0)
        ...
        negative_difftop1_std = negative_difftop1 + 1.5 * \
            np.std(negative_numpy[negative_numpy < negative_difftop1])
    ...
    def get_peaktop(self, negative_numpy, bw_adjust, x_lab):
        ...
        negative_maxstdtop1 = negative_maxtop1 + 1.5 * \
            np.std(negative_numpy[negative_numpy < negative_maxtop1])
    ...
    def getpeak(self, negative_group, positive_group):
        ...
        cross_top1, cross_top2 = self.get_crosspoint(
            negative_density, positive_density, x_lab)

        # DBSCAN
        negative_DBSCANtop = self.getDBSCAN(
            negative_numpy, self.eps_score, self.my_min_samples)
        positive_DBSCANtop = self.getDBSCAN(
            positive_numpy, self.eps_score, self.my_min_samples)
    ...
    def peak_HR(self, peak_x, negative_group, positvie_group, prefix):
        if peak_x == 0:
            redf = collections.OrderedDict({prefix + '_cut': [0],
                                        prefix + '_neg_HR': [1],
                                        prefix + '_neg_pvalue': [1],
                                        prefix + '_neg_1': [0],
                                        prefix + '_neg_1_mscore': [0],
                                        prefix + '_neg_0': [0],
                                        prefix + '_neg_0_mscore': [0],
                                        prefix + '_pos_HR': [1],
                                        prefix + '_pos_pvalue': [1],
                                        prefix + '_pos_1': [0],
                                        prefix + '_pos_1_mscore': [0],
                                        prefix + '_pos_0': [0],
                                        prefix + '_pos_0_mscore': [0]})
            return redf
        ...
        negative_group.loc[:, 'myfpkm_level'] = negative_group.fpkm.apply(
            lambda a: 1 if (a < peak_x) else 0)
        positvie_group.loc[:, 'myfpkm_level'] = positvie_group.fpkm.apply(
            lambda a: 1 if (a < peak_x) else 0)
    ...
    def HR_corr(self, gene, mydata):
        ...
        mycut_neg_HR = (
            b[b.columns[b.columns.str.endswith('_neg_HR')]]).to_numpy()[0]
        mycut_pos_HR = (
            b[b.columns[b.columns.str.endswith('_pos_HR')]]).to_numpy()[0]

        neg_1_mscore = (
            b[b.columns[b.columns.str.endswith('_neg_1_mscore')]]).to_numpy()[0]
        pos_1_mscore = (
            b[b.columns[b.columns.str.endswith('_pos_1_mscore')]]).to_numpy()[0]
        ...
        cut_HR_pearsonr_neg = scipy.stats.pearsonr(
            cutHR.mycut_numpy, cutHR.log_neg_HR)
        cut_HR_spearmanr_neg = scipy.stats.spearmanr(
            cutHR.mycut_numpy, cutHR.log_neg_HR)

        socre_HR_pearsonr_neg = scipy.stats.pearsonr(
            cutHR.neg_1_mscore, cutHR.log_neg_HR)
        socre_HR_spearmanr_neg = scipy.stats.spearmanr(
            cutHR.neg_1_mscore, cutHR.log_neg_HR)

        cut_HR_pearsonr_pos = scipy.stats.pearsonr(
            cutHR.mycut_numpy, cutHR.log_pos_HR)
        cut_HR_spearmanr_pos = scipy.stats.spearmanr(
            cutHR.mycut_numpy, cutHR.log_pos_HR)

        socre_HR_pearsonr_pos = scipy.stats.pearsonr(
            cutHR.pos_1_mscore, cutHR.log_pos_HR)
        socre_HR_spearmanr_pos = scipy.stats.spearmanr(
            cutHR.pos_1_mscore, cutHR.log_pos_HR)
        ...
        neg_pvalue = (
            b[b.columns[b.columns.str.endswith('_neg_pvalue')]]).to_numpy()[0]
        pos_pvalue = (
            b[b.columns[b.columns.str.endswith('_pos_pvalue')]]).to_numpy()[0]
        ...
        if not np.isnan(min(cutoff_pos_HR)):
            pos_cut_percent = min(
                pos_mypercent[np.where(mycut == min(cutoff_pos_HR))])
            neg_cut_percent = min(
                neg_mypercent[np.where(mycut == min(cutoff_pos_HR))])
        else:
            pos_cut_percent = np.nan
            neg_cut_percent = np.nan

        try:
            cut_mscore_HR_neg = pg.linear_regression(
                cutHR[['mycut_numpy', 'neg_1_mscore']], cutHR['log_neg_HR'], remove_na=True)
            cut_mscore_HR_neg_r2 = cut_mscore_HR_neg.adj_r2[0]
        except:
            cut_mscore_HR_neg_r2 = 0
        try:
            cut_mscore_HR_pos = pg.linear_regression(
                cutHR[['mycut_numpy', 'pos_1_mscore']], cutHR['log_pos_HR'], remove_na=True)
    ...
    def distrution_pipelinemap(self, genefpkm):
        ...
        my_dict = {'fpkm':genefpkm,'death_observed':self.mydata_survival.death_observed,'survivalMonth':self.mydata_survival.survivalMonth,'myscore':self.mydata_survival.myscore,'mylevel':self.mydata_survival.mylevel}
        c1 = pd.DataFrame(my_dict)
        ...
        ttest = scipy.stats.ttest_ind(
            positive_value, negative_value, equal_var=False)
        ...
        negative_top1std_HR = self.peak_HR(
            negative_top1std, negative_group, positive_group, 'negative_top1std')
        negative_top2_HR = self.peak_HR(
            negative_top2, negative_group, positive_group, 'negative_top2')

        positive_top1_HR = self.peak_HR(
            positive_top1, negative_group, positive_group, 'positive_top1')
        positive_top1std_HR = self.peak_HR(
            positive_top1std, negative_group, positive_group, 'positive_top1std')
        positive_top2_HR = self.peak_HR(
            positive_top2, negative_group, positive_group, 'positive_top2')

        negative_difftop1_HR = self.peak_HR(
            negative_difftop1, negative_group, positive_group, 'negative_difftop1')
        negative_difftop1_std_HR = self.peak_HR(negative_difftop1_std, negative_group, positive_group,
                                                'negative_difftop1_std')
        negative_difftop2_HR = self.peak_HR(
            negative_difftop2, negative_group, positive_group, 'negative_difftop2')

        positive_difftop1_HR = self.peak_HR(
            positive_difftop1, negative_group, positive_group, 'positive_difftop1')
        positive_difftop1_std_HR = self.peak_HR(positive_difftop1_std, negative_group, positive_group,
                                                'positive_difftop1_std')
        positive_difftop2_HR = self.peak_HR(
            positive_difftop2, negative_group, positive_group, 'positive_difftop2')

        cross_top1_HR = self.peak_HR(
            cross_top1, negative_group, positive_group, 'cross_top1')
        cross_top2_HR = self.peak_HR(
            cross_top2, negative_group, positive_group, 'cross_top2')

        negative_DBSCANtop_HR = self.peak_HR(
            negative_DBSCANtop, negative_group, positive_group, 'negative_DBSCANtop')
        positive_DBSCANtop_HR = self.peak_HR(
            positive_DBSCANtop, negative_group, positive_group, 'positive_DBSCANtop')

        allq10_df_HR = self.peak_HR(
            allq10, negative_group, positive_group, 'allq10')
        allq25_df_HR = self.peak_HR(
            allq25, negative_group, positive_group, 'allq25')
        allq50_df_HR = self.peak_HR(
            allq50, negative_group, positive_group, 'allq50')
        allq75_df_HR = self.peak_HR(
            allq75, negative_group, positive_group, 'allq75')
        allq90_df_HR = self.peak_HR(
            allq90, negative_group, positive_group, 'allq90')

        allmean_df_HR = self.peak_HR(
            allmean, negative_group, positive_group, 'allmean')
        allmeanq90_df_HR = self.peak_HR(
            allmeanq90, negative_group, positive_group, 'allmeanq90')
        all2FPKM_df_HR = self.peak_HR(
            all2FPKM, negative_group, positive_group, 'all2FPKM')
        ...
        if self.judgeconstant(positive_value, negative_value):
            HRcorr_df = pd.DataFrame({'gene': gene,
                 'cut_HR_pearsonr_neg_r': 0,
                 'cut_HR_pearsonr_neg_pvalue': 1,
                 'cut_HR_spearmanr_neg_r': 0,
                 'cut_HR_spearmanr_neg_pvalue': 1,

                 'cut_HR_pearsonr_pos_r': 0,
                 'cut_HR_pearsonr_pos_pvalue': 1,
                 'cut_HR_spearmanr_pos_r': 0,
                 'cut_HR_spearmanr_pos_pvalue': 1,

                 'socre_HR_pearsonr_neg_r': 0,
                 'socre_HR_pearsonr_neg_pvalue': 1,
                 'socre_HR_spearmanr_neg_r': 0,
                 'socre_HR_spearmanr_neg_pvalue': 1,

                 'socre_HR_pearsonr_pos_r': 0,
                 'socre_HR_pearsonr_pos_pvalue': 1,
                 'socre_HR_spearmanr_pos_r': 0,
                 'socre_HR_spearmanr_pos_pvalue': 1,

                 'cut_mscore_HR_neg_r2': 0,
                 'cut_mscore_HR_pos_r2': 1,
            ...
            all_result['ttest'] = 1
            all_result['pearson'] = 0
            all_result['spearman'] = 0
        else:
        ...
        return all_result.iloc[0,:]


'''
# test
mydata_FPKM6 = pd.read_pickle(
    '/share_storage/home/nacoo/enst/mydata_FPKM6.pickle')
start = time.time()
warnings.filterwarnings("ignore")

#gene='5S_rRNA_ENST00000364415.1'
gene = 'RTN3_ENST00000537981.5_1'
result = RNA_groupsurvival(mydata_FPKM6).distrution_pipelinemap(gene, 'death_observed', 'survivalMonth', 'myscore', 'mylevel')
print(result)
end = time.time()
print(end - start)
result.to_csv('/share_storage/home/nacoo/enst/r3.xls', sep='\t')
'''
